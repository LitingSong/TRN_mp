...
# footprint file
trn_footprint <- read.table('~/GTRD/macs2_fdr/liver.bed', sep='\t')
...
print(4)
liver_hts <- findOverlaps(tf_j_gr, TSS_10k)
liver_hts <- as.data.frame(liver_hts)

liver_hts$queryHits <- tf_j[liver_hts$queryHits,'motif_alt_id']
liver_hts$subjectHits <- grch38[liver_hts$subjectHits, 'symbol' ]
liver_hts <- unique(liver_hts)
liver_hts11 <- liver_hts
#brain_hts <- brain_hts_o

save(liver_hts11, file='~/GTRD/final_gtrd/liver/liver_hts11.RData')
