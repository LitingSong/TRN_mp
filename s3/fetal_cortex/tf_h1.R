...
J1 <- read.table('~/GTRD/brain_trn/Jaspar_coords/h1.tsv',sep='\t',header = T)
...
fetal_brain_hts$queryHits <- tf_j[fetal_brain_hts$queryHits,'motif_id']
fetal_brain_hts$subjectHits <- grch38[fetal_brain_hts$subjectHits, 'symbol' ]
fetal_brain_hts <- unique(fetal_brain_hts)

fetal_brain_hts_h1 <- fetal_brain_hts
#fetal_brain_hts <- fetal_brain_hts_o

save(fetal_brain_hts_h1, file='~/GTRD/final_gtrd/fetal_brain/fetal_brain_hts_h1.RData')
