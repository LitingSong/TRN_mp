...
J1 <- read.table('~/GTRD/brain_trn/Jaspar_coords/j11.tsv',sep='\t',header = T)

# footprint file
trn_footprint <- read.table('~/GTRD/macs2_fdr/fetal_brain.bed', sep='\t')
...
# findOverlaps,findOverlaps with the +-10kb region surrounding a gene transcriptional starting site.
print(4)
fetal_brain_hts <- findOverlaps(tf_j_gr, TSS_10k)
fetal_brain_hts <- as.data.frame(fetal_brain_hts)

fetal_brain_hts$queryHits <- tf_j[fetal_brain_hts$queryHits,'motif_alt_id']
fetal_brain_hts$subjectHits <- grch38[fetal_brain_hts$subjectHits, 'symbol' ]
fetal_brain_hts <- unique(fetal_brain_hts)

fetal_brain_hts11 <- fetal_brain_hts
#fetal_brain_hts <- fetal_brain_hts_o

save(fetal_brain_hts11, file='~/GTRD/final_gtrd/fetal_brain/fetal_brain_hts11.RData')
